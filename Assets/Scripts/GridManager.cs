...
public class GridManager : MonoBehaviour
{
    ...
    [Header("Scene Scanning")]
    [Tooltip("Parent transforms whose children should be marked as permanent at runtime (e.g., a 'ManualBuildings' empty).")]
    public Transform[] manualPlacementParents;
    ...
    /// <summary>
    /// At runtime, scans all children of manualPlacementParents and marks
    /// their grid cells as permanently occupied. Use this for buildings
    /// you manually place in the scene (not generated by CityGenerator/RoadGenerator).
    /// </summary>
    private void MarkManualPlacementsAtRuntime()
    {
        // Auto-find ManualBuildings if the array was not assigned in the inspector
        if (manualPlacementParents == null || manualPlacementParents.Length == 0)
        {
            GameObject mb = GameObject.Find("ManualBuildings");
            if (mb != null)
            {
                manualPlacementParents = new Transform[] { mb.transform };
                Debug.Log("[GridManager] Auto-found 'ManualBuildings' parent.");
            }
            else
            {
                Debug.Log("[GridManager] No ManualBuildings parent found — skipping manual placement scan.");
                return;
            }
        }
        ...
        foreach (Transform parent in manualPlacementParents)
        {
            ...
            foreach (Transform child in parent)
            {
                ...
                totalMarked++;
                Debug.Log($"[GridManager] Marked '{child.name}' as permanent — bounds ({b.min.x:F1},{b.min.z:F1})-({b.max.x:F1},{b.max.z:F1}) → cells ({minX},{minZ})-({maxX},{maxZ})");
            }
        }

        Debug.Log($"[GridManager] Marked {totalMarked} manually placed objects as permanent.");
    }
