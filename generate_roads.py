"""
Generate placeholder road prefabs, materials, and CityTileData assets for road generation.
Road types: RoadStart, RoadEnd, RoadFiller, RoadIntersection, RoadBranch.
"""
import os
import uuid
import random
...
# Road definitions: (name, sizeInCells, scaleX, scaleY, scaleZ, colorR, colorG, colorB)
ROADS = [
    ("RoadStart",        1,  2.4, 0.15, 2.4,  0.2,  0.8,  0.2 ),  # Green  — start marker
    ("RoadEnd",          1,  2.4, 0.15, 2.4,  0.85, 0.15, 0.15),  # Red    — end marker
    ("RoadFiller",       1,  2.4, 0.1,  4.8,  0.5,  0.5,  0.5 ),  # Gray   — straight segment
    ("RoadIntersection", 1,  2.4, 0.1,  2.4,  0.95, 0.9,  0.2 ),  # Yellow — 4-way crossing
    ("RoadBranch",       1,  2.4, 0.1,  2.4,  0.95, 0.55, 0.15),  # Orange — T-junction
]
...
# ── Prefab template (road: Y offset 0.05 to sit on terrain) ───────
...
def main():
    ...
    prefab_dir = os.path.join(BASE, "Prefabs", "Environment")
    ...
    for (name, size, sx, sy, sz, r, g, b) in ROADS:
        # 1. Material
        mat_guid = make_guid()
        ...
        with open(mat_path, "w", newline="\n") as f:
            f.write(make_material(mat_name, r, g, b))
        ...
        with open(asset_path, "w", newline="\n") as f:
            f.write(make_city_tile_data(name, name, size, prefab_guid, prefab_root_id))
    ...
    print(f"\nDone! Created road assets for: {', '.join(created)}")
